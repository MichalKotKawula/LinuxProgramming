...
int register_device(void)
{
    ...
    printk( KERN_NOTICE "Hardware-Device: registered character device with major number = %i and minor numbers 0...255\n"
        , device_file_major_number );//TODO: Function prototypes for the read and ioctl functions
...
static ssize_t hardware_device_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    ...
    // If offset is more than buffer length
    if (*off >= BUF_LEN)
        return 0;

    //check min number of bytes to read
    bytes_to_read = min(len, (size_t)(BUF_LEN - *off));

    //copy data from kernel to user space
    bytes_not_copied = copy_to_user(buf, buffer + *off, bytes_to_read);
    
    //check if they were successfully copied
    if (bytes_not_copied) {
        printk(KERN_WARNING "Failed to copy bytes to user\n", bytes_not_copied);
    }

    // add bytes to file offset
    *off += bytes_to_read;
    // calculate the number of bytes read
    bytes_read = bytes_to_read - bytes_not_copied;

    printk(KERN_INFO "Hardware_device_read: called, %zu bytes read\n", bytes_read);
    return bytes_read;
}
...
static long hardware_device_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    //switch statement to handle ioctl based on cmd
    switch (cmd) {
    
        case HARDWARE_DEVICE_HALT:
            is_halt = true;
            printk(KERN_INFO "Hardware_device_ioctl: HARDWARE_DEVICE_HALT\n");
            break;
            
        case HARDWARE_DEVICE_RESUME:
            is_halt = false;
            printk(KERN_INFO "Hardware_device_ioctl: HARDWARE_DEVICE_RESUME\n");
            break;
